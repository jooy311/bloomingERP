					...
					<table id="data-table-command" class="table table-striped table-vmiddle" aria-busy="false">
					    ...
					    <tbody>
					    	...
					    	<c:choose>
								...
								<c:otherwise>
							    	<c:forEach var="rcpUp" items="${rcpListUp}">
							    		<tr>
							    			<td>${rcpUp.rcpNo}</td>
							    			<td>${rcpUp.rcpProduct}</td>
							    			<td>${rcpUp.rcpDate}</td>
							    			<td>${rcpUp.rcpCnt}</td>
							    			<td>${rcpUp.rcpPrice}</td>
							    			<td>${rcpUp.rcpMemo}</td>
							    			<td>
								    			<button type="button" 
								    				...
								    				name="cltNo" 
								    				value="${rcp.rcpNo}" 
								    				onclick="location.href='${pageContext.request.contextPath}/stock/repUpdate?rcpNo=${rcp.rcpNo}';">
					...
					<div class="row">
						...
						<div class="col-xs-6 col-md-4">
							<div class="fg-line form-group">
								...
								<input type="hidden" class="form-control input-sm" placeholder="발주번호를 입력해주세요" name="poNo" id="poNo" required>
							</div>
						</div>
		...
		$("#data-table-command").bootgrid({
			formatters: {
                "commands": function(column, row) {
                    return "<button type=\"button\" class=\"btn btn-icon command-edit\" data-row-id=\"" + row.id + "\" onclick=\"location.href='${pageContext.request.contextPath}/stock/repUpdate?rcpNo=" + row.id + "';\"><span class=\"md md-edit\"></span></button>"; 
